export class AudioEngine {
  ...
  unlock() {
    ...
    try {
      ...
      /* Sound is optional when an audio device is unavailable. */
    }
  }
  ...
  play(event) {
    ...
    if (event.type === 'shot' && now - this.lastShot > 70) {
      this.lastShot = now;
      this.tone(530, 0.055, 'triangle', 0.027, 180);
    }
    if (event.type === 'kill')
      this.tone(event.boss ? 65 : 120, event.boss ? 0.65 : 0.12, 'triangle', 0.055, 35);
    if (event.type === 'pickup' && now - this.lastPickup > 100) {
      this.lastPickup = now;
      this.tone(event.heal ? 660 : 1000, 0.07, 'sine', 0.021, 1350);
    }
    ...
    if (event.type === 'boss' || event.type === 'lost' || event.type === 'timeout')
      this.tone(160, 0.6, 'triangle', 0.06, 55);
  }
}
