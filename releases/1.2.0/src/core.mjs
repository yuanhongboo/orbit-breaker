import { CONFIG, ENEMIES, UPGRADES, VIEW } from './config.mjs';
import { isTargetVisible, spawnPosition } from './view.mjs';
...
export function createGame(seed = Date.now()) {
  return {
    ...
    player: {
      ...
      fireInterval: 0.31,
      projectiles: 1,
      magnet: CONFIG.baseMagnetRadius,
      movementAngle: -Math.PI / 2,
      dashLeft: 0,
      dashCooldown: 0,
      dashMax: CONFIG.dashCooldown,
      dashX: 0,
      dashY: -1,
      invulnerable: 1.6,
      orbitCount: 0,
    ...
    stats: { shots: 0, hits: 0, dashes: 0, damageTaken: 0, bestCombo: 0, volleys: 0 },
  };
}
...
export function spawnEnemy(game, kind, position) {
  ...
  if (!position && game.view) {
    let best,
      bestDistance = -1;
    for (let attempt = 0; attempt < 12; attempt++) {
      const candidate = spawnPosition(game, spec.radius);
      candidate.x = clamp(candidate.x, spec.radius + 20, CONFIG.worldWidth - spec.radius - 20);
      candidate.y = clamp(candidate.y, spec.radius + 20, CONFIG.worldHeight - spec.radius - 20);
      const d = distance(candidate, game.player);
      if (d > bestDistance) {
        best = candidate;
        bestDistance = d;
      }
      if (d >= VIEW.minimumSpawnDistance) break;
    }
    position = best;
  }
  const x = clamp(
    position?.x ?? game.player.x + Math.cos(angle) * radius,
    20,
    CONFIG.worldWidth - 20,
  );
  const y = clamp(
    position?.y ?? game.player.y + Math.sin(angle) * radius,
    20,
...
export function hurtPlayer(game, damage, source) {
  const p = game.player;
  ...
  emit(game, 'damage', {
    ...
    y: p.y,
    amount: damage,
    sourceAngle: source ? Math.atan2(source.y - p.y, source.x - p.x) : null,
  });
...
function hitEnemy(game, enemy, amount) {
  ...
  enemy.hp -= amount;
  enemy.flash = 0.085;
  emit(game, 'impact', { x: enemy.x, y: enemy.y });
  game.stats.hits++;
  ...
  emit(game, 'kill', {
    ...
    score: spec.score * game.multiplier,
  });
  if (enemy.kind === 'boss') {
...
export function dash(game, dx, dy) {
  ...
  p.dashX = length > 0.01 ? dx / length : Math.cos(p.movementAngle);
  p.dashY = length > 0.01 ? dy / length : Math.sin(p.movementAngle);
...
export function projectileOffsets(count, volley = 0) {
  const offsets = [0];
  const side = volley % 2 ? -1 : 1;
  for (let i = 1; i < count; i++)
    offsets.push(Math.ceil(i / 2) * (i % 2 ? side : -side) * CONFIG.projectileFanStep);
  return offsets;
}
...
function shoot(game) {
  ...
  for (const enemy of game.enemies) {
    ...
    if (enemy.alive && d < bestDistance && isTargetVisible(game, enemy)) {
      target = enemy;
      bestDistance = d;
    }
  }
  ...
  p.angle = angle;
  const offsets = projectileOffsets(p.projectiles, game.stats.volleys++);
  for (let i = 0; i < offsets.length && game.bullets.length < CONFIG.maxBullets; i++) {
    const spread = offsets[i];
...
export function step(game, input, dt = CONFIG.step) {
  ...
  if (length > 0.1) p.movementAngle = Math.atan2(dy, dx);
  if (length > 0.1 && !game.enemies.length) p.angle = Math.atan2(dy, dx);
  const smooth = 1 - Math.exp(-dt * CONFIG.cameraFollowRate);
  game.camera.x += (p.x - game.camera.x) * smooth;
  game.camera.y += (p.y - game.camera.y) * smooth;
  if (game.time > game.comboUntil) {
    game.combo = 0;
    game.multiplier = 1;
  }
  ...
  for (const enemy of game.enemies) {
    ...
    if (enemy.kind === 'gunner' && d < 335 && isTargetVisible(game, enemy))
      speed *= d < 230 ? -0.45 : 0.05;
    if (enemy.kind === 'boss' && d < 290 && isTargetVisible(game, enemy)) speed = 0;
    ...
    if (d < enemy.radius + p.radius) {
      hurtPlayer(game, enemy.kind === 'boss' ? 35 : enemy.kind === 'tank' ? 28 : 16, enemy);
      if (game.status !== 'playing') return;
      if (p.dashLeft <= 0) {
    ...
    if (enemy.kind === 'gunner' || enemy.kind === 'boss') {
      enemy.shotIn -= dt;
      if (enemy.shotIn <= 0) {
        enemyShot(game, enemy);
        enemy.shotIn = enemy.kind === 'boss' ? 1.4 : 2.8;
      }
    }
    if (p.orbitCount && game.time - enemy.orbitHitAt > 0.34) {
      for (let i = 0; i < p.orbitCount; i++) {
        ...
        if (
          Math.hypot(enemy.x - p.x - Math.cos(a) * 90, enemy.y - p.y - Math.sin(a) * 90) <
          enemy.radius + 12
        ) {
  ...
  for (const bullet of game.bullets) {
    ...
    for (const enemy of game.enemies) {
      if (
        enemy.alive &&
        segmentHits(oldX, oldY, bullet.x, bullet.y, enemy.x, enemy.y, enemy.radius + 4)
      ) {
  ...
  for (const bullet of game.hostileBullets) {
    ...
    if (segmentHits(oldX, oldY, bullet.x, bullet.y, p.x, p.y, bullet.radius + p.radius)) {
      hurtPlayer(game, 13, { x: p.x - bullet.vx, y: p.y - bullet.vy });
      bullet.life = 0;
    }
    if (game.status !== 'playing') return;
  }
  ...
  if (p.novaLevel) {
    ...
    if (game.novaIn <= 0) {
      ...
      for (const enemy of game.enemies)
        if (distance(enemy, p) < radius + enemy.radius) hitEnemy(game, enemy, 50 * p.novaLevel);
      game.novaIn = 4;
    }
  }
  if (game.status !== 'playing') return;
...
export function safeProfile(raw) {
  ...
  try {
    ...
    if (Array.isArray(parsed.settledRunIds))
      empty.settledRunIds = parsed.settledRunIds
        .filter((id) => typeof id === 'string' && /^[a-zA-Z0-9-]{1,80}$/.test(id))
        .slice(0, 20);
    if (Array.isArray(parsed.records))
      empty.records = parsed.records
        .filter(
          (r) =>
            r &&
    ...
    /* A corrupt or unavailable local save starts a fresh profile. */
  }
  return empty;
}
